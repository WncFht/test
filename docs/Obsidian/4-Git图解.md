...
modify: 2024-12-06T00:11:11+0800
categories: Computer
dir: Obsidian
share: true
tags:
...
这样，`_maint_ `分支就不再是 `_master_` 分支的祖父节点。此时，[merge](4-Git%E5%9B%BE%E8%A7%A3.md##merge) 或者 [rebase](4-Git%E5%9B%BE%E8%A7%A3.md##rebase) 是必须的。
...
另一个例子是[分离HEAD提交](4-Git%E5%9B%BE%E8%A7%A3.md##detached)，在后面的章节中介绍。
...
如果既没有指定文件名，也没有指定分支名，而是一个标签、远程分支、SHA-1 值或者是像 `_master~3_` 类似的东西，就得到一个匿名分支，称作 `_detached HEAD_`（被分离的 `_HEAD_` 标识）。这样可以很方便地在历史版本之间互相切换。比如说你想要编译 1.6.6.1 版本的 Git，你可以运行 `git checkout v1.6.6.1`（这是一个标签，而非分支名），编译，安装，然后切换回另一个分支，比如说 `git checkout master`。然而，当提交操作涉及到「分离的 HEAD」时，其行为会略有不同，详情见在[下面](4-Git%E5%9B%BE%E8%A7%A3.md.md##detached)。
...
如果给了文件名(或者 `-p` 选项), 那么工作效果和带文件名的[checkout](4-Git%E5%9B%BE%E8%A7%A3.md##checkout)差不多，除了索引被更新。
...
本质上，这是线性化的自动的 [cherry-pick](4-Git%E5%9B%BE%E8%A7%A3.md##cherry-pick)。
